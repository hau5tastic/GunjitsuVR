...
namespace Wacki {
    abstract public class IUILaserPointer : MonoBehaviour {
        ...
        void Start()
        {
            // Hacky solution. This script draws the laser pointer relative to the VR Controller,
            // but we want it drawn relative to our Revolver model, which has a rotational offset.
            ...
            hitPoint.transform.SetParent(controllerTransform, false);
            hitPoint.transform.localScale = new Vector3(laserHitScale, laserHitScale, laserHitScale);
            hitPoint.transform.localPosition = new Vector3(100.0f, 0.0f, 0.0f);
